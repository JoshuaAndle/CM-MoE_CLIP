...
def load(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit=True, design_details={}, log_dir=None):
    """Load a CLIP model
    Parameters
    ----------
    name : str
    ...
    try:
        model = build_model(state_dict or model.state_dict(), design_details, log_dir).to(device)
    except KeyError:
        sd = {k[7:]: v for k, v in state_dict["state_dict"].items()}
        model = build_model(sd, design_details).to(device)
...
def tokenize(texts: Union[str, List[str]], context_length: int = 77, force_eot: bool = False) -> torch.LongTensor:
    """
