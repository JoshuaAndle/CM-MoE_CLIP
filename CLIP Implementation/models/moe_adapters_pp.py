...
class moe_adapters_pp(nn.Module):

    def __init__(self, args, model_name, peft_method='lora', peft_encoder='image', device='cpu', log_dir=None):
        super(moe_adapters_pp, self).__init__()
        self.device = device
    ...
    def forward(self, image=None, text_tokens=None, apply_softmax=True, val_text_task_id=None):
        ...
        if image is None or text_tokens is None:
            ...
            # logits_per_image, _, image_features, text_features = self.clip_model(image, text_tokens, val_text_task_id=val_text_task_id)
            # if apply_softmax:
